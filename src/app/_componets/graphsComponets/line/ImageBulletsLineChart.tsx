...
export default function ImageBulletsLineChart() {
    ...
    useLayoutEffect(() => {
        const IBLChartRoot = am5.Root.new("ImageBulletsLineChartdiv");
        const colors = am5.ColorSet.new(IBLChartRoot, {});

        // Set themes
        IBLChartRoot.setThemes([am5themes_Animated.new(IBLChartRoot)]);

        // Create chart
        const chart = IBLChartRoot.container.children.push(am5xy.XYChart.new(IBLChartRoot, { panX: false, panY: false, layout: IBLChartRoot.verticalLayout, paddingBottom: 50, paddingLeft: 20, paddingRight: 20, paddingTop: 50 }));
        ...
        // Create axes
        const xRenderer = am5xy.AxisRendererX.new(IBLChartRoot, { minorGridEnabled: true, minGridDistance: 30 });
        ...
        const xAxis = chart.xAxes.push(am5xy.CategoryAxis.new(IBLChartRoot, { categoryField: 'date', maxDeviation: 0, renderer: xRenderer, }));

        const yRenderer = am5xy.AxisRendererY.new(IBLChartRoot, { strokeOpacity: 0.1, });
        ...
        const yAxis = chart.yAxes.push(am5xy.ValueAxis.new(IBLChartRoot, { extraMax: 0.05, min: 0, max: 100, strictMinMax: true, numberFormat: "#'%'", renderer: yRenderer }));
        ...
        // Add legend
        const legend = chart.children.push(am5.Legend.new(IBLChartRoot, { clickTarget: "none", dy: 20, centerX: am5.p50, x: am5.p50, y: am5.percent(100), useDefaultMarker: true, centerY: am5.percent(100), layout: IBLChartRoot.horizontalLayout }));
        ...
        const createSeries = (Legendname: string, data: Array<dataType>, image: imageType, colorIndex: number) => {
            // Add series
            const series = chart.series.push(am5xy.LineSeries.new(IBLChartRoot, {
                name: Legendname,
                xAxis: xAxis,
                yAxis: yAxis,
                valueYField: "value",
            ...
            series.bullets.push(function () {
                const container = am5.Container.new(IBLChartRoot, { centerX: am5.p50, centerY: am5.p50 });
                container.children.push(am5.Picture.new(IBLChartRoot, { centerX: am5.p50, centerY: am5.p50, width: 50, height: 50, src: image.src }));
                const circle = am5.Circle.new(IBLChartRoot, { centerX: am5.p50, centerY: am5.p50, radius: 35, stroke: series.get("fill"), fill: am5.color("#FFFFFF"), strokeWidth: 2 });
        ...
        const colorIndex = [9, 12, 0, 17, 41];
        const images = [godzilla, hydra, kraken, minotaur, dragon]
        ...
        const cursor = chart.set("cursor", am5xy.XYCursor.new(IBLChartRoot, {}));
        ...
        //Exporting
        const exporting = am5plugins_exporting.Exporting.new(IBLChartRoot, {
