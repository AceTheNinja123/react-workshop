...
interface dataType { date: string; value: number; }
...
export default function GradientChart() {
    ...
    useLayoutEffect(() => {
        const gradientRoot = am5.Root.new("gradientchartdiv");
        const colors = am5.ColorSet.new(gradientRoot, { step: 2 });

        // Set themes
        gradientRoot.setThemes([am5themes_Animated.new(gradientRoot)]);

        // Create chart
        const chart = gradientRoot.container.children.push(am5xy.XYChart.new(gradientRoot, { panX: false, panY: false, layout: gradientRoot.verticalLayout, paddingBottom: 50, paddingLeft: 20, paddingRight: 20, paddingTop: 50 }));
        ...
        // Create axes
        const xRenderer = am5xy.AxisRendererX.new(gradientRoot, { minorGridEnabled: true, minGridDistance: 80 });
        ...
        const xAxis = chart.xAxes.push(am5xy.CategoryAxis.new(gradientRoot, { categoryField: 'date', maxDeviation: 0, renderer: xRenderer, tooltip: am5.Tooltip.new(gradientRoot, {}) }));

        const yRenderer = am5xy.AxisRendererY.new(gradientRoot, { strokeOpacity: 0.1, });
        ...
        const yAxis = chart.yAxes.push(am5xy.ValueAxis.new(gradientRoot, { extraMax: 0.1, renderer: yRenderer }));

        // Add legend
        /*const legend = chart.children.push(am5.Legend.new(gradientRoot, {clickTarget:"none", dy: 20, centerX: am5.p50, x: am5.p50, centerY: am5.percent(100), y: am5.percent(100)}));
        ...
        legend.itemContainers.template.events.on("pointerover", function(e) {
            ...
            const bullets = chartSeries.bullets;
        ...
        const createSeries = (name: string, data: dataType[]) => {
            const color = colors.next();
            // Add series
            const series = chart.series.push(am5xy.SmoothedXLineSeries.new(gradientRoot, {
                name: name,
                xAxis: xAxis,
                yAxis: yAxis,
                valueYField: "value",
        ...
        const cursor = chart.set("cursor", am5xy.XYCursor.new(gradientRoot, {}));
        ...
        //Exporting
        const exporting = am5plugins_exporting.Exporting.new(gradientRoot, {
